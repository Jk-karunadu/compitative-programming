...
int main() {
    ...
    if (sumt < min_total || sumt > max_total) {
        cout << "NO" << endl;
        return 0;
    }
    ...
    vector<int> schedule = mn;
    int current_sum = min_total;
    int extra_needed = sumt - current_sum;

    for (int i = 0; i < d; i++) {
        if (extra_needed <= 0) break;

        int can_add = mx[i] - mn[i];
        
        int to_add = min(extra_needed, can_add);
