...
long long cntMod(long long L, long long R, int rem)
{
    if (L > R) return 0;

    long long first = L;
    long long cur = first % 4;
    if (cur < 0) cur += 4;

    long long add = (rem - cur + 4) % 4;
    first += add;
...
void solve()
{
    ...
    long long left0 = 1 + cntMod(1, x - 1, 3);
    long long left1 = cntMod(0, x - 1, 1);

    long long right0 = cntMod(x, n, 3);
    long long right1 = cntMod(x, n, 1);
